...
public class X {
	...
	private void loadFrameTransformMatrix(String str) {
		...
		for (int i = 0; i < 15; ++i) {
			...
			transformMatrix.m[i] = Float.parseFloat(str.substring(valueBegin,
					valueEnd));
		...
		valueEnd = str.indexOf(";", valueBegin);
		transformMatrix.m[15] = Float.parseFloat(str.substring(valueBegin,
				valueEnd));
	}
	...
	public void loadTPose(String s) {
		...
		for (int i = 0; i < vertexNumber; ++i) {
			for (int j = 0; j < 3; ++j) {
				...
				vertexArray[i * 3 + j] = Float.parseFloat(s.substring(begin,
						end));
			}
			++end;
		}
		...
		for (int i = 0; i < textureCoordNumber; ++i) {
			for (int j = 0; j < 2; ++j) {
				...
				textureCoordArray[i * 2 + j] = Float.parseFloat(s.substring(
						begin, end));
			}
			++end;
		}
	...
	public void draw(GL10 gl) {
		...
		gl.glDrawElements(GL10.GL_TRIANGLES, indexNumber * 3,
